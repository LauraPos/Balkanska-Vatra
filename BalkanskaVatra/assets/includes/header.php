<nav class="fixed top-0 left-0 right-0 z-30 bg-plum-950/85 backdrop-blur-xl border-b border-gold/20">
  <div class="max-w-7xl mx-auto px-6 py-5 flex items-center justify-between">
    ...
    <div class="flex items-center gap-3">
      ...
      <!-- ✅ VERANDERING: Admin link checkt nu of je ingelogd bent -->
      <!-- Als je ingelogd bent: gaat naar admin.php (geen login nodig) -->
      <!-- Als je niet ingelogd bent: gaat naar login.php -->
      <a href="<?php echo (isset($_SESSION['admin_logged_in']) && $_SESSION['admin_logged_in']) ? 'admin.php' : 'login.php'; ?>" class="hidden md:inline-block px-3 py-2 border border-plum-600/40 text-plum-500 font-cinzel text-[10px] tracking-widest hover:border-plum-500 hover:text-plum-300 transition-all duration-300">Admin</a>
    </div>
  </div>
</nav>