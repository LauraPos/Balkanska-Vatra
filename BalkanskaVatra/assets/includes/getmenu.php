...
?>
    <div class="menu-card opacity-0 transition-opacity duration-500 group relative bg-plum-900/60 border border-gold/15 hover:border-gold/45 hover:-translate-y-1 hover:shadow-[0_20px_50px_rgba(0,0,0,0.5)]" data-gang="<?= htmlspecialchars($row['gang']) ?>">
        ...
        <div class="p-7">
            ...
            <button
                onclick="addToCart(<?= json_encode($row['naam']) ?>, <?= $row['prijs'] ?>)"
                class="w-full py-2.5 border border-gold/30 text-gold font-cinzel text-xs tracking-widest hover:bg-gold hover:text-plum-950 transition-all duration-200 flex items-center justify-center gap-2">
                Add to Order
            </button>
        </div>
    </div>
